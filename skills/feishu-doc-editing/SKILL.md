...
本 skill 封装飞书云文档的编辑操作流程与踩坑。**飞书是战略会研究的交付面**（专题报告/选题跟踪都在飞书维护），本地只存过程与快照。需要编辑飞书文档时按本 skill 操作。
...
classifier(glm-5.2) 间歇阻塞 Bash/PowerShell 写操作（报 "auto mode cannot determine safety"，只读命令仍可用），可持续 15+ 分钟。
